...
cv2.imshow("eroded image", 255*ErodedEllipseKernel)
cv2.waitKey(0)

# Create a VideoWriter object
videoWriter = cv2.VideoWriter(
    "erosionScratch.avi", cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10, (50, 50))
...
for h_i in range(border, height+border):
    for w_i in range(border, width+border):
        ...
        product = cv2.multiply(
            ...
            element[indexes])
        paddedErodedIm[h_i, w_i] = np.amin(product)

        # Resize output to 50x50 before writing it to the video
        resizedFrame = cv2.resize(
            255*paddedErodedIm, (50, 50), interpolation=cv2.INTER_NEAREST)
        ...
        # cv2.imshow("intermediate result", 255*paddedErodedIm)
        # cv2.waitKey(0)
...
while key != 27:
    cv2.imshow("method2", 255 *
               paddedErodedIm[border: height + border, border: width + border])
